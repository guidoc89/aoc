...
@main def part2(): Int = {
  ...
  for (line <- lines) {
    ...
    for (i <- 0 until line.length) {
      if (line.slice(i, l_length).startsWith("do()")) {
        enabled = true
      } else if (line.slice(i, l_length).startsWith("don't()")) {
        enabled = false
      } else {
        if (line.slice(i, l_length).startsWith("mul(")) {
          ...
          if (splits.length == 2 && splits(0).toIntOption.isDefined && splits(1).toIntOption.isDefined && enabled) {
